...
void 环城::寻环()
{
    ...
    总点数++;
    队.push_back(父);
    sort(队.begin(), 队.end(), [](点 甲, 点 乙)
         { return 甲.界 > 乙.界; }); // 按界降序,左大右小,右侧出队
    cout << "入(" << 父.层 << "," << 父.列 << ")\n"
         << endl;
    while (!队.empty())
    {
        ...
        if (父.界 < 最短路长)
        {
            ...
            if (队.size() > 0)
            {
                ...
                for (int i = 0; i <= 队.size()-1; i++)
                {
                    if (队[i].界 >= 最短路长)
                    {
                        cout << ">灭(" << 队[i].层 << "," << 队[i].列 << ")" << endl;
                        小++;
                    }
                }
                队.erase(队.begin(), 队.begin() + 小);
            }
            // 队.erase(remove_if(队.begin(), 队.end(), [=](点 d){ 
            //     cout << ">灭(" << d.层 << "," << d.列 << ")" << endl;
            //     return (d.界>=最短路长);}));
            // cout << endl;
        }
