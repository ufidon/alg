...
void 环城::寻环()
{
    ...
    while (!队.empty())
    {
        ...
        cout << "Dequeue node (" << 父.层 << "," << 父.列 << ")" << endl;
        出队点数++;
        父.界 = 界(父);
        if (父.界 < 最短路长)
        {
            ...
            for (int 陌 : 父陌)
            {
                ...
                if (子.层 == 城数 - 2)
                {
                    ...
                    cout << "Get a tour at node ";
                    印点(子);
                }
                else
                {
                    cout << "Visit and calculate node ";
                    印点(子);
                    if (子.界 < 最短路长)
                    {
                        队.push(子);
...
void 环城::印环() const
{
    ...
    cout << "Total No. of nodes:" << 总点数 << endl;
}
...
void 环城::印队(const priority_queue<点, vector<点>, 比界> &队)
{
    ...
    cout << "Current nodes in the queue: {";
    while (!d.empty())
    {
...
{
    //本点.列 = 列号[本点.层];
    cout << "(" << 本点.层 << "," << 本点.列 << ") with part tour ";
    印路(本点.路);
    cout << " partlength =[" << 路长(本点.路);
    cout << "], bound = [" << 本点.界 << "], minlength = [" << 最短路长 << "]" << endl;
    //列号[本点.层]++;
}
